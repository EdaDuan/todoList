 ...
 * @LastEditors: duanfy
 * @LastEditTime: 2021-08-29 20:56:53
 */
import {
  removeEmptyBox,
  listEmpty,
...
let selectAllTodo = document.querySelector("#selectAllTodo");
let selectAllDone = document.querySelector("#selectAllDone");
...
const changeSelectAllStatus = (status) => {
  let beforMoveUl = status ? conTodoUl : conDoneUl;
  let alterMoveUl = status ? conDoneUl : conTodoUl;
  let beforMoveSelectAll = status ? selectAllTodo : selectAllDone;
  let alterMoveSelectAll = status ? selectAllDone : selectAllTodo;
  let beforMoveLabel = status ? taskLabel[0] : taskLabel[1];
  let alterMoveLabel = status ? taskLabel[1] : taskLabel[0];
  let length = beforMoveUl.childNodes.length;
  ...
  beforMoveUl.appendChild(
    emptyBox(status ? "今日任务已全部完成～" : "今日还未完成任务～")
  );
};
// 复选框
...
const changTodaySelectStatus = (element, isEdit) => {
  ...
  let appendUl = status ? conDoneUl : conTodoUl;
  let beforMoveSelectAll = status ? selectAllTodo : selectAllDone;
  let alterMoveSelectAll = status ? selectAllDone : selectAllTodo;
  let beforMoveLabel = status ? taskLabel[0] : taskLabel[1];
  let alterMoveLabel = status ? taskLabel[1] : taskLabel[0];
  let text = status ? "今日任务已全部完成～" : "今日还未完成任务～";
  ...
  // changeStatus(isClickTodo, element, isEdit);
};
// 未完成状态
...
const changeTodayDelStatus = (element) => {
  //  判断是否为最后一项,为最后一项，在点击之后需要添加空盒子和给全选按钮加状态
  let targetUlDom = element.target.parentNode.parentNode;
  if (targetUlDom.childNodes.length === 1) {
    let status = targetUlDom.id === "con-todo-ul" ? true : false;
    let targetSelectAll = status ? selectAllTodo : selectAllDone;
    let targetLabel = status ? taskLabel[0] : taskLabel[1];
    let text = status ? "今日任务已全部完成～" : "今日还未完成任务～";
...
const changeRecycleStatus = (element) => {
  let ulNode = element.target.parentNode.parentNode;
